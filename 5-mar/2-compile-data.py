...
def compile_dfs(column):
    ...
    for year in years:
        ...
        # Import data
        df1 = pd.read_csv(path +'mar_results/results_' + str(year) + '.csv', 
    ...
    df2 = pd.read_csv(path +'mar_results/results_' + str(idx) + '.csv', 
                      index_col=['grid_cell_i', 'grid_cell_j'])
...
# Save
all_stats.to_csv(path + 'mar_stats.csv', index=False)
...
slope1, intercept1, r_value1, p_value1, std_err1 = stats.linregress(t1, e1)
slope2, intercept2, r_value2, p_value2, std_err2 = stats.linregress(t2, e2)
slope3, intercept3, r_value3, p_value3, std_err3 = stats.linregress(t3, e3)
slope4, intercept4, r_value4, p_value4, std_err4 = stats.linregress(s1, e1)
slope5, intercept5, r_value5, p_value5, std_err5 = stats.linregress(s2, e2)
slope6, intercept6, r_value6, p_value6, std_err6 = stats.linregress(s3, e3)
...
ax1.scatter(t1, e1, color='#c1272d', zorder=2, s=75, alpha=0.6)
...
ax2.scatter(t2, e2, color='#c1272d', zorder=2, s=75, alpha=0.6)
...
ax3.scatter(t3, e3, color='#c1272d', zorder=2, s=75, alpha=0.6)
...
ax4.scatter(s1, e1, color='#0000a7', zorder=2, s=75, alpha=0.6)
...
ax5.scatter(s2, e2, color='#0000a7', zorder=2, s=75, alpha=0.6)
...
ax6.scatter(s3, e3, color='#0000a7', zorder=2, s=75, alpha=0.6)
...
ax2.set_xlabel('Mean PDD June (K)', fontsize=14)
ax3.set_xlabel('Mean PDD June (K)', fontsize=14)
ax4.set_xlabel('Max snow depth (m)', fontsize=14)
ax5.set_xlabel('Max snow depth (m)', fontsize=14)
ax6.set_xlabel('Max snow depth (m)', fontsize=14)
...
ax1.text(0.3, 1.01, "Southwest", fontsize=16, transform=ax1.transAxes)
ax2.text(0.3, 1.01, "Central West", fontsize=16, transform=ax2.transAxes)
ax3.text(0.3, 1.01, "North", fontsize=16, transform=ax3.transAxes)
...
is_summary.to_csv(path + 'mar_summary_stats.csv', index=False)
...
ax1.hist(all_stats['coeffs_t'], bins=bins, color='#c1272d', 
         lw=1.5, alpha=0.6, zorder=3, edgecolor='k', label='Air temp.')
ax1.hist(all_stats['coeffs_s'], bins=bins, color='#0000a7', 
         lw=1.5, alpha=0.6, zorder=3, edgecolor='k', label='Snow depth')

ax1.axvline(x=np.mean(all_stats['coeffs_t']), lw=2, ls='dashed', c='k', zorder=4)
ax1.axvline(x=np.mean(all_stats['coeffs_s']), lw=2, ls='dashed', c='k', zorder=4)

ax1.set_ylabel('Frequency', fontsize=14)
ax1.set_xlabel('Glacier ice exposure sensitivity', fontsize=14)
...
for r in np.arange(1, 9, 1):
    ...
    stats_region.append([region_stats.shape[0],
                         np.mean(t_region_stats, axis=1).mean(),
                         np.mean(s_region_stats, axis=1).mean(),
                         np.mean(e_region_stats, axis=1).mean(),
                         np.mean(t_region_stats, axis=1).std(),
                         np.mean(s_region_stats, axis=1).std(),
                         np.mean(e_region_stats, axis=1).std(),
...
region_names = ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW']
region_df = pd.DataFrame(stats_region, columns=['count', 'mean_t', 'mean_s', 'mean_e', 
                                                'std_t', 'std_s', 'std_e', 
                                                'r2_t', 'r2_s', 
                                                'slope_t', 'slope_s'])
...
fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=(10, 4), 
                               layout='constrained')
ax1.scatter(region_df['mean_s'], region_df['slope_t'], s=100, color='#0000a7', zorder=2)
for i, txt in enumerate(region_names):
    ax1.annotate(txt, (region_df['mean_s'].iloc[i], region_df['slope_t'].iloc[i]), fontsize=14)

ax2.scatter(region_df['mean_s'], region_df['slope_s'], s=100, color='#0000a7', zorder=2)
for i, txt in enumerate(region_names):
...
ax1.set_ylabel('Sensitivity to June PDD (days)', fontsize=14)

ax2.set_xlabel('Mean max. snow depth 2001-2021 (m)', fontsize=14)
ax2.set_ylabel('Sensitivity to snow depth (days)', fontsize=14)
...
#%%
###############################################################################
# Relationship between Spring air temperature and max snow depth?
###############################################################################
...
for i in range(all_jm.shape[0]):
    ...
    t_vs_sd.append([p_value1, r_value1])

stats_df = pd.DataFrame(t_vs_sd, columns=['p', 'r'])
...
region_names = ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW']
t_vs_sd_df = pd.DataFrame(percent, columns=['num_sig', 'num_all', 'fraction'])
t_vs_sd_df['region'] = region_names

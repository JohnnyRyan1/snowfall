...
for i in files[0:200]:
    ...
    # Check if empty
    if valid_lon.shape[0] > 0:
        # Append
        lon_data.append(valid_lon)
        lat_data.append(valid_lat)
        snow_data.append(valid_snow)
        time_data.append(valid_time)
...
flat_snow = np.concatenate(snow_data).ravel()
flat_time = np.concatenate(time_data).ravel()

# Make a GeoDataFrame
gdf = gpd.GeoDataFrame(list(zip(flat_lon, flat_lat, flat_snow, flat_time)))
...
# Save
gdf.to_file(fp + 'cloudsat-inglefield.shp')
